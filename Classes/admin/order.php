...
<?php
    class Order
    {
        ...
        {
            // $query = "SELECT * FROM `order` o JOIN `account` a ON o.IdAccount = a.IdAccount ORDER BY `o.DateOrder` DESC";
            // $query = "SELECT * FROM `order` o JOIN `account` a ON o.IdAccount = a.IdAccount ORDER BY `o.DateOrder` DESC";
             $query =" SELECT 
                            o.*, 
                            a.*
                        FROM 
                            `order` o 
                        JOIN 
                            `account` a ON o.IdAccount = a.IdAccount 
                        ORDER BY 
                            o.DateOrder DESC";
        ...
        {
            $query = "SELECT * FROM `order` ORDER BY `DateOrder` DESC";
            $result = $this->db->select($query);
            $total = 0;
